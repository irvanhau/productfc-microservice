...
func (r *ProductRepository) FindProductByID(ctx context.Context, productID int64) (*models.Product, error) {
	...
	err := r.Database.WithContext(ctx).Table("products").Where("id = ?", productID).Last(&product).Error
	if err != nil {
		return nil, err
	}
...
func (r *ProductRepository) DeleteProduct(ctx context.Context, productID int64) error {
	err := r.Database.WithContext(ctx).Table("products").Delete(&models.Product{}, productID).Error
	if err != nil {
		return err
	}
	return nil
